...
class BlackjackGUI:
    def __init__(self, env, agent):
        ...
        self.card_animation = []  # (card_image, start_pos, end_pos, progress)
    ...
    def draw_game_state(self):
        ...
        self.draw_text(f"Score: {calculate_hand_value(self.env.get_hand('HUMAN'))}", 100, 580)
        ...
        self.draw_text(f"Score: {calculate_hand_value(self.env.get_hand('AI'))}", 740, 580)
        self.draw_text(f"Chips: €{self.env.chips['AI']}", 740, 610)

        if self.ai_action_timer > 0:
            self.draw_text(self.ai_action_text, 740, 370, (255, 255, 0), font=self.large_font)
        ...
        for anim in self.card_animation:
            self.animate_card(*anim)

    def animate_card(self, image, start_pos, end_pos, progress):
    ...
    def run_game(self):
        while not self.done:
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    if event.key == pygame.K_h:
                        self.env.step("HUMAN", 0)
                        self.human_turn = not self.env.done["HUMAN"]
                    elif event.key == pygame.K_s:
                        self.env.step("HUMAN", 1)
                        self.human_turn = False
            ...
            if not self.awaiting_bet and not self.human_turn and not self.env.done['AI']:
                ...
                self.ai_action_text = "AI: HIT" if ai_action == 0 else "AI: STAND"
